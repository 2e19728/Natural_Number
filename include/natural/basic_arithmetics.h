...
extern "C" {
	void nat_mul_basecase_odd(uint64_t len2, uint64_t len1, const uint64_t* src2, const uint64_t* src1, uint64_t* dst);
	void nat_mul_basecase_even(uint64_t len2, uint64_t len1, const uint64_t* src2, const uint64_t* src1, uint64_t* dst);
	// dst[0 .. 2n-1] = src[0 .. n-1]^2, n >= 3 (see mul_basecase.s)
	void nat_sqr_basecase(uint64_t n, const uint64_t* src, uint64_t* dst);
}
...
inline void mul_basecase_default(uint64_t* dst, const uint64_t* src1, const uint64_t* src2, uint64_t len1, uint64_t len2) {
	if (len2 & 1)
		nat_mul_basecase_odd(len2, len1, src2, src1, dst);
	else
		nat_mul_basecase_even(len2, len1, src2, src1, dst);
}
...
// carries a single nat_sqr_basecase entry.  (The even/odd split in that file exists only for
