...
# ---------------- asmNtt_radix4: m4q + root==1 first block special-cased -------------
# Same two-layer merged pass as asmNtt2_radix4, but with the first 4D block (block index
# 0, where the outer-layer twiddle w and the first inner-layer twiddle w2 are both 1)
# handled without any twiddle multiply, mirroring what asmNtt does vs asmNtt2.
...
# The generic remainder jumps into the asmNtt2_radix4 block loop (.Lm4q_block), so this
...
.Lr4f_group:
	...
	dec	rcx
	jnz	.Lr4f_group
	# ---- past the special block: continue with the generic blocks of asmNtt2_radix4
	mov	rax, r11
	lea	rax, [rax+rax*2]
	add	r8, rax
	add	r9, rax
...
# ---------------- asmINtt2_radix4: merged inverse layers D then 2D ----------------
...
# ---------------- asmINtt_radix4: inverse m4q + root==1 first block special-cased --
# Same two-layer merged pass as asmINtt2_radix4, but the first 4D block (block index 0:
# stage-D twiddle w2 == 1 on (a,b) and stage-2D twiddle w == 1 on the inner pairs)
# is handled without twiddle multiplies, mirroring asmINtt vs asmINtt2.
...
# The generic remainder jumps into the asmINtt2_radix4 block loop (.Li4_block).
	.globl	asmINtt_radix4
asmINtt_radix4:
	push	rbx
	push	rbp
	push	r12
...
.Lr4i_group:
	...
	dec	rcx
	jnz	.Lr4i_group
	# ---- past the special block: continue with the generic blocks of asmINtt2_radix4
	mov	rax, r11
	lea	rax, [rax+rax*2]
	add	r8, rax
	add	r9, rax
...
# ---------------- asmNtt2: single-layer DIF forward pass (generic, 2 butterflies/iter)
#   void asmNtt2(uint64_t N, uint64_t mod, uint64_t* Begin, const uint64_t* Root, uint64_t* End)
...
#   Register state after entry shuffle (shared with asmNtt):
...
# ---------------- asmNtt: asmNtt2 + root==1 first block no-multiply ----------------
# Same single-layer pass as asmNtt2, but block 0 (root value 1) uses the multiply-free
# butterfly: u'=red(u), v'=red(v), lo=u'+v', hi=u'+m-v'.  After the special block the
# generic remainder jumps into asmNtt2's block loop (.Lnt2_block), sharing its
...
.Lnt_sp:
	...
	dec	rcx
	jnz	.Lnt_sp
	# ---- advance to block 1 and jump into asmNtt2's generic block loop ----
	lea	r8, [r8+rsi*8]
	add	r9, 16
	cmp	r8, rbp
	jae	.Lnt2_fin
...
# ---------------- asmINtt2: single-layer DIT inverse pass (generic, 2 butterflies/iter)
#   void asmINtt2(uint64_t N, uint64_t mod, uint64_t* Begin, const uint64_t* Root, uint64_t* End)
...
# ---------------- asmINtt: asmINtt2 + root==1 first block no-multiply ----------------
# Same single-layer inverse pass as asmINtt2, but block 0 (root value 1) uses the
# multiply-free inverse butterfly: lo=red(u+v), hi=(u-v) mod m.  The generic remainder
# jumps into asmINtt2's block loop (.Lit2_block), sharing its epilogue (.Lit2_fin);
...
.Lit_sp:
	...
	dec	rcx
	jnz	.Lit_sp
	# ---- advance to block 1 and jump into asmINtt2's generic block loop ----
	lea	r8, [r8+rsi*8]
	add	r9, 16
	cmp	r8, rbp
	jae	.Lit2_fin
...
# ================================ asmNttMul ======================================
# void asmNttMul(uint64_t N, uint64_t* Dst, const uint64_t* Src1, const uint64_t* Src2,
...
#
# This is the same algorithm as the original MASM asmCRT (bench/asmCRT_orig.s, checked
